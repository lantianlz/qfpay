...
def send_email(emails, title, content, type='text'):
    ...
    emails = [email for email in emails if not 'mrqfpay' in email]
    if type != 'html':
...
def replace_href_to_open_blank(content):
    ...
    def _re_sub(match):
        ...
        atag = match.group(0)
        href = match.group(1)
        if 'chinaxbpenlyh.com' not in href:
...
def get_function_code(func):
    return '%s_%s' % (func.__name__, func.func_code.co_filename.split('qfpay')[-1].replace('/', '_').replace('\\', '_').replace(".py", ""))
