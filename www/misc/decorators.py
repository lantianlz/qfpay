...
def channel_required(func):
    def _decorator(request, *args, **kwargs):
        if not request.session.get('CHANNEL_ID'):
            return HttpResponseRedirect("/admin/choose_channel")
    ...
    def _decorator(request, *args, **kwargs):
        ...
        if not (hasattr(request, 'user') and request.user.is_authenticated()):
            if request.is_ajax():
                ...
                #     err_msg = u'需要登陆后进行操作<br /><br />请在微信中搜索公众号「联谊会」，关注后通过菜单访问'
...
def company_manager_required_for_request(func):
    ...
    def _decorator(request, company_id, *args, **kwargs):
        ...
        if not is_cm:
            if request.is_ajax():
                return HttpResponse('{}')
            err_msg = u'权限不足，你还不是公司管理员，如有疑问请联系联谊会客服'
            return render_to_response('error.html', locals(), context_instance=RequestContext(request))
